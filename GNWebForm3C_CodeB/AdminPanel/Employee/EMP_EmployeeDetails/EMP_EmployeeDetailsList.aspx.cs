...
public partial class AdminPanel_Employee_EMP_EmployeeDetails_EMP_EmployeeDetailsList : System.Web.UI.Page
{
    #region 11.0 Variables

    String FormName = "EMP_EmployeeList";
    ...
    private void Search(int PageNo)
    {
        ...
        DataTable dt = balEMP_EmployeeDetails.SelectPage(Offset, PageRecordSize, out TotalRecords, EmployeeName, EmployeeTypeID);
    ...
    protected void rpData_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "DeleteRecord")
        {
            try
            {
                EMP_EmployeeDetailsBAL balMST_EmployeeDetails = new EMP_EmployeeDetailsBAL();
                if (e.CommandArgument.ToString().Trim() != "")
                {
                    if (balMST_EmployeeDetails.Delete(Convert.ToInt32(e.CommandArgument)))
                    {
    ...
    protected void lbtnExport_Click(object sender, EventArgs e)
    {
        ...
        EMP_EmployeeDetailsBAL balEMP_EmployeeDetails = new EMP_EmployeeDetailsBAL();
        DataTable dtEMP_EmployeeDetails = balEMP_EmployeeDetails.SelectPage(Offset, PageRecordSize, out TotalReceivedRecord, EmployeeName, EmployeeTypeID);
        if (dtEMP_EmployeeDetails != null && dtEMP_EmployeeDetails.Rows.Count > 0)
        {
            Session["ExportTable"] = dtEMP_EmployeeDetails;
